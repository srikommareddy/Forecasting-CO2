...
model_SARIMA = pickle.load(pickle_in)
...
def main():       
    ...
    if st.sidebar.button("SARIMA Model"):
        future_df_SARIMA['forecast_SARIMA']= model_SARIMA.predict(start = future_df_SARIMA.index[145], end = 145 + f_period) 
        fpred = pd.DataFrame(future_df_SARIMA["forecast_SARIMA"][145:])
        st.title("Forecasted CO2 Data by SARIMA Model")
        st.dataframe(fpred)
        st.line_chart(future_df_SARIMA)   
